...
- **技能SOP库** — 四套研究playbook（估值、财务报表、行业对比、风险扫描），一个分类步骤一次可以加载一到两个，注入进规划prompt，真正改变会问哪些子问题——用真实的前后对比评测验证过，不只是prompt拼接测试（详见[量化评测](#量化评测)）。
...
| 指标 | 结果 | 依据 |
|---|---|---|
| RAG检索/回答准确率 | 14/14 = 100% | 基于事实的问答对，针对实际的财报/新闻/科普语料核对（另有1题因测试设计有缺陷被排除，见`eval/report.md`） |
| 工具路由准确率（宽松——所需工具有被调用） | 17/17 = 100% | 涵盖finance_query/rag_search/web_search/无需工具/复合意图的问题 |
| 工具路由准确率（严格——工具集合完全匹配） | 10/17 = 58.8% | 同一数据集；这个差距反映的是Reflect循环里一个真实的、已记录的过度触发倾向，不是路由bug——所需工具总会被调用，但`web_search`经常被额外（且没必要地）当作兜底触发 |
| 响应延迟 | 首字节平均2.1秒；完整响应 p50 29.9秒 / p90 85.6秒 | 32次真实流式请求；多轮Reflect的case拉长了长尾 |
| 边界场景健壮性 | 4/4 = 100%（1个case结果不确定，见报告） | 空输入、超长输入、无效股票代码、不存在的session——均被优雅处理，没有5xx或未捕获异常 |
| SOP注入提升（单命中，估值类） | 基线34.3% → 注入后69.4% = **+35.2个百分点**（阈值≥30个百分点）→ 通过 | 12道纯估值问题，每种条件采样3次取平均，真实的`qwen-plus`调用，走实际的`agent_plan()` |
| SOP注入提升（双命中，两个SOP都要有提升） | 已提交的这次运行7题中6题通过（见`eval/skill_coverage_run.txt`） | 7道期望同时命中2个SOP的复合问题；唯一未通过的一题（`sop-dual-02`）是两种条件下用词完全相同的泛泛表述（"MSFT基本面"）在某一维度上从6%打平到0%——诊断为评分盲区+单条件采样噪音，不是功能性回归（同一问题的*另一个*SOP维度从17%跳到了100%）。复核时用一批全新的3次采样独立重跑，7/7全部通过——跟"噪音"这个诊断吻合，不是用一次"修好的"结果覆盖了这次的结论 |

方法论、完整数据集和原始记录：[`eval/report.md`](eval/report.md)、[`eval/dataset.py`](eval/dataset.py)、[`eval/results_final.json`](eval/results_final.json)。用`python eval/run_eval.py`针对运行中的后端重新跑一遍。

SOP覆盖率的方法论、数据集和逐题原始记录：[`eval/skill_coverage_eval.py`](eval/skill_coverage_eval.py)、[`eval/skill_coverage_dataset.py`](eval/skill_coverage_dataset.py)、[`eval/skill_coverage_raw.json`](eval/skill_coverage_raw.json)。用`python eval/skill_coverage_eval.py`重新跑（需要真实的DashScope key；约170次真实的`qwen-plus`调用，每种条件采样3次）。

**诚实说明：** 语料库是这个项目自带的测试数据（部分由AI生成，日期设定在未来）——RAG准确率衡量的是对已索引语料的忠实度，不是针对真实世界金融数据的验证。

## 已知局限

- **Reflect循环里的工具过度触发** — 如上面测得的，agent的严格工具路由准确率（58.8%）明显落后于宽松准确率（100%）：它总会稳定调用*所需*的工具，但经常额外调用`web_search`作为不必要的兜底，增加了延迟/成本但不影响正确性。这是一个真实的效率优化目标，不是路由bug。
