...
需要外部信息时，**直接发起工具调用**。不要用文字去描述你打算查什么——写下"需要检索…"
或"需要外部信息"却不发起调用，系统读到的就是"这一轮什么都不做"，用户会拿到一个没有
依据的回答。想查就调，两者不是二选一。

只有在不需要再取数时才使用文字输出：要么是面向用户的回复，要么是一句"为什么已经足够"
的理由，它会展示给用户。
"""
...
def final_answer(query: str, language: str = "auto", history: list[dict] | None = None, session_id: str | None = None, user_profile: dict | None = None):
    ...
    #
    # 为什么需要它：分阶段实测（eval/stage_timing.py，n=36）显示，到第一条「正在调用 …」
    # 为止的耗时里，第 1 轮决策这一次 LLM 往返独占 99.9%——中位数 2.222s、均值 2.361s、
    # 范围 1.149-5.175s；其余全部阶段（语言判断、结论召回、DB 读、轨迹拼装）合计约 3ms。
    # 也就是说「第一条消息必须携带真实规划结果」这个设计，天然把首次反馈锁死在一次 LLM
    # 往返之上，换模型或改提示词都消不掉这段等待。
    #
    # 数字随代码演进过，改这段前先重跑，不要直接沿用：增量三把 classify_skill 并入主模型
    # 的 SOP 目录后，这条路径上从两次 LLM 往返变成一次（数据同步更新于 e874e4e）。复现：
    #     cd backend/app && PYTHONPATH=. python ../../eval/stage_timing.py --repeats 3
    # 该脚本不经过 HTTP/SSE，未计网络栈与 Starlette 线程池调度，端到端首次反馈略高于此。
    #
    # 拆成两级之后两个时刻各自有明确含义，也各自可优化：
    #   第一级 受理回执    用户多久知道系统在动          不等 LLM，毫秒级
    #   第二级 正在调用 …  系统多久拿出第一个实质判断    仍在一次 LLM 往返之后
...
## 语气与长度
- 常规回答控制在 300-600 字：先用一两句给出结论，再给支撑它的依据
- 宁可少写一个维度，也不要把已经说过的内容换个说法再说一遍
- 无法回答、或问题超出范围时，用 1-2 句说清楚就停，不要解释自己为什么做不到
- 加粗只留给真正关键的数字或结论；通篇加粗等于没有重点
