...
## 重要限制：在仍有去重的代码上跑，本脚本看不见重复

`seen_prompts` 是在 yield 提示消息**之前**过滤的，被它拦下的重复根本不会进入 SSE。
所以在 master 或增量一那种去重仍在的 commit 上，本脚本的「严格重复」必然为 0，
那个 0 不代表模型没试图重复。此时唯一有效的信号在后端日志里：

    grep -c '均为重复请求' <backend.log>     # 模型试图重复（整轮全是重复）
    grep -c '安全上限'     <backend.log>     # 循环空转到上限

并且 `均为重复请求` 只在**一轮里所有动作都是重复**时才打印，部分重复不记录，
因此它给出的是重复尝试次数的**下界**。
