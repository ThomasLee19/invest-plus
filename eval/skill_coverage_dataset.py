...
# ── 深循环题集：会把决策循环压到多轮的问题 ──────────────────────────────────────
# 为什么需要它：主评测的路由/RAG 题多为单一事实查询，实测 37 题产生 **0 次补充调用**，
# 循环一次都没迭代过。任何关于多轮行为的性质（重复触发、轨迹增长、上下文膨胀）在这样的
# 题集上结构性地测不到——"没测到"和"不发生"是两回事。
# 挑选标准是"一次取数答不完"：需要多个面向、或需要跨主体对比。实测这批能驱动出
# 每题 2-6 次补充调用（对照：主评测题集为 0）。
DEEP_LOOP = [
